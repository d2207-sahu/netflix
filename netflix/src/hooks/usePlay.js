...
import useFirebaseMovieList from './useFirebaseMovieList';
...
const usePlay = ({ onAfterClick, movieID, videoID, movieData }) => {
  ...
  const { saveMovieToList } = useFirebaseMovieList({keyword :'played'});
  ...
  const onPlayClick = () => {
    if (movieID) {
      ...
      if (videoID) dispatch(updateModalSelectedVideo(videoID));
      if (onAfterClick) onAfterClick();
    }
  };
